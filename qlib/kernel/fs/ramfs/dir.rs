...
pub struct CreateOps {
    ...
    pub NewBoundEndpoint: Option<
        fn(task: &Task, dir: &Inode, ep: &BoundEndpoint, perms: &FilePermissions) -> Result<Inode>,
    >,
...
impl DirInternal {
    ...
    fn createInodeOperationsCommon(
        &mut self,
        task: &Task,
        name: &str,
        makeInodeOperations: &mut FnMut() -> Result<Inode>,
    ) -> Result<Inode> {
...
impl InodeOperations for Dir {
    ...
    fn Bind(
        ...
        let newep = match d.CreateOps.NewBoundEndpoint {
            None => return Err(Error::SysError(SysErr::EACCES)),
            Some(NewBoundEndpoint) => NewBoundEndpoint,
        };
...
impl FileOperations for DirFileOperation {
    ...
    fn Seek(&self, task: &Task, f: &File, whence: i32, current: i64, offset: i64) -> Result<i64> {
        return SeekWithDirCursor(
            task,
            f,
            whence,
            current,
            offset,
            Some(&mut self.dirCursor.lock()),
        );
    }
...
pub fn Rename(
    ...
    if replacement {
        let replaced = npLocked
            .children
            .get(newName)
            .expect("Dirent claims rename is replacement, the newName is not find");
